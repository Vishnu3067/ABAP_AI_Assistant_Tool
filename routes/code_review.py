...
from core.models import CodeReviewRequest
from core.sap_client import get_session, resolve_url, fetch_source, fetch_reviewable_source, extract_and_resolve_data_sources
from core.prompts import ai_code_review
...
async def code_review(payload: CodeReviewRequest):
    ...
    try:
        analysis = ai_code_review(
            payload.artifact_type, payload.artifact_name, system_id,
            source_code,
            resolved_types=extract_and_resolve_data_sources(get_session(), sap_url, source_code),
        )
    except Exception as e:
